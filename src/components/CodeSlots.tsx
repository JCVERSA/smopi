...
const digitsOf = (raw: any) => String(raw ?? '').replace(/[\s]/g, '');
const toSlots = (raw: any, n: number) => {
...
export default function CodeSlots({
  ...
  const [veiled, setVeiled] = useState<boolean>(status === 'success');
  ...
  const drainTimer = useRef<any>(undefined);
  const statusRef = useRef(status);
  const emitted = useRef<string>(digitsOf(value ?? defaultValue).slice(0, length));
  const slotsRef = useRef(slots);
  slotsRef.current = slots;
  const live = useRef<{ settle: number; bounce: number; cascade: number; reduce: boolean | null }>({
    settle,
    bounce,
    cascade,
    reduce: !!reduce
  });
  live.current = { settle, bounce, cascade, reduce: !!reduce };
  ...
  const insert = (raw: any, from = active) => {
  ...
  useEffect(() => {
    ...
    if (status === 'success') {
      ...
      animate(openMv as any, 1, { duration: WASH_IN, ease: EASE_OUT as any });
      drops.forEach((d, k) =>
        animate(d as any, 1, { type: 'spring', duration: 0.3, bounce: 0, delay: (SINK_DELAY + k * SINK_STEP) })
      );
      animate(checkMv as any, 1, { type: 'spring', duration: 0.35, bounce: L.bounce, delay: CHECK_DELAY });
      return;
    }
    ...
    animate(checkMv as any, 0, { duration: 0.15, ease: EASE_OUT as any });
    const anim = animate(openMv as any, 0, { duration: WASH_OUT, ease: EASE_OUT as any, delay: 0.06 });
    Promise.resolve(anim).then(() => {
      if (openMv.get() === 0) setVeiled(false);
    });
    drops.forEach(d => animate(d as any, 0, { type: 'spring', duration: 0.3, bounce: 0, delay: 0.1 }));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [status]);
...
function Slot({
  ...
  const fill = useTransform(mv, (t: any) => `scale(${Math.max(t, 0)})`);
  const lift = useTransform([mv, drop], ([t, d]: any) => `translateY(${(1 - t) * rise + Math.max(d, 0) * sink}px)`);
  const ink = useTransform([mv, drop], ([t, d]: any) => clamp01(t) * (1 - clamp01(d / SINK_FADE)));
  return (
    <span
