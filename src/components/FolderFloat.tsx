...
const DEFAULT_ITEMS: string[] = ['Try a warmer palette', 'Tighten the spacing', 'Logo feels small', 'Love the new hero'];
const PAD = 28;
const CHAR = 6.8;
const GAP = 12;
const ROW = 52;
...
const layout = (list: FolderFloatItem[], spread: number, lift: number, tilt: number, sizes: { w: number; h: number }[]) => {
...
export default function FolderFloat({
  ...
  const latest = useRef<{ onSelect?: any; onOpenChange?: any; drift: number; reduce: boolean }>({
    onSelect,
    onOpenChange,
    drift,
    reduce: false
  ...
  const popTimer = useRef<any>(undefined);
  const liveTimer = useRef<any>(undefined);
  const list: FolderFloatItem[] = items.map(item => (typeof item === 'string' ? { label: item, value: item } : item));
  const n = list.length;
  ...
  useLayoutEffect(() => {
    const measure = () => {
      ...
      setSizes(prev =>
        prev.length === next.length && prev.every((s, i) => s.w === (next[i] as any).w && s.h === (next[i] as any).h)
          ? prev
  ...
  const up = (e: React.PointerEvent, i: number, item: FolderFloatItem) => {
    const w = world.current;
    const d = w.drag;
    if (!d || d.i !== i || d.id !== e.pointerId) return;
    w.drag = null;
