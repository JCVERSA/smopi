...
export default function SquishSwitch({
  ...
  const scaleX = useTransform([flow, swell], ([v, h]: any) => stretchOf(v) * h);
  const scaleY = useTransform([flow, swell], ([v, h]: any) => h / stretchOf(v));
  ...
  const up = (e: any, cancelled: boolean) => {
    const g = grip.current;
    if (!g || g.id !== e.pointerId) return;
    grip.current = null;
    try {
