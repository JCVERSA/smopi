...
const wallIt = (t: number, room: number, wall: 'clamp' | 'bounce') => {
  if (wall === 'bounce') {
...
const bearingOf = (dx: number, dy: number, d: number, axis: 'both' | 'x' | 'y') => {
  if (axis === 'x') return { x: Math.sign(dx) || 1, y: 0 };
  if (axis === 'y') return { x: 0, y: Math.sign(dy) || 1 };
  return { x: dx / d, y: dy / d };
};

export interface DodgeFieldState {
  dodges: number;
  gave: boolean;
  caught: boolean;
  fleeing: boolean;
...
export interface DodgeFieldProps {
  children?: React.ReactNode | ((state: DodgeFieldState) => React.ReactNode);
  taunts?: string[];
  notice?: string;
  inkColor?: string;
...
export default function DodgeField({
  ...
  const raf = useRef(0);
  const hold = useRef<any>(undefined);
  const live = useRef<{
    reach: number;
    radius: number;
    falloff: number;
  ...
  const frame = useCallback(() => {
    ...
    if (L.reduce) {
      ...
      animate(x, tx, cfg as any);
      animate(y, ty, cfg as any);
    }
  ...
  return (
    <div
      ...
      style={{ '--df-ink': inkColor, '--df-contrast': contrastColor, '--df-height': `${fieldHeight}px`, ...style } as React.CSSProperties}
    >
      <motion.div
        ref={moverRef}
