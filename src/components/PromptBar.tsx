...
const DEFAULT_SOURCES: PromptSource[] = [
  ...
  { key: 'sales', name: 'Sales data', description: 'Revenue and churn', icon: LineChart },
...
function SendGlyph({ busy, morphDuration, squash, tilt }: { busy: boolean; morphDuration: number; squash: number; tilt: number }) {
  ...
  useEffect(() => {
    ...
    const controls = animate(
      t,
      target,
      reduce ? { duration: 0 } : { duration: morphDuration / 1000, ease: EASE_IN_OUT as any }
    );
...
export interface PromptBarProps {
  ...
  busy?: boolean;
  onSend?: (text: string, meta: { attachments: string[]; model: PromptModel; effort: string }) => void;
  onStop?: () => void;
  onAttach?: () => Promise<string | string[] | undefined> | string | string[] | undefined;
  onDictate?: () => Promise<string | undefined> | string | undefined;
  background?: string;
  color?: string;
...
export default function PromptBar({
  ...
  const latest = useRef<{
    ...
    onDictate?: any;
    onEffortChange?: any;
  }>({ onSend, onStop, onAttach, onDictate, onEffortChange });
  ...
  const effortFromPointer = (e: React.PointerEvent<HTMLDivElement>) => {
          ...
          {open === 'effort' ? (
            <>
              ...
              <div
                ...
                style={{ '--pb-effort-x': stepAt(effortIndex), '--pb-effort-fill': fillAt(effortIndex) } as React.CSSProperties}
                onPointerDown={e => {
