...
export interface ThoughtLineProps {
  ...
  glyph?: 'sparkle' | 'dot' | 'orb' | 'none' | React.ReactNode;
  orbState?: OrbState;
  steps?: string[];
  collapsible?: boolean;
...
export default function ThoughtLine({
  ...
  const latest = useRef<{ onSettle?: (s: number) => void }>({ onSettle });
  ...
  useEffect(() => {
    ...
    const loop = (el: HTMLElement, delay: number) =>
      animate(el, { opacity: [trough, 1, trough] }, { duration: period, ease: EASE_IN_OUT as any, repeat: Infinity, delay });
    let cancelled = false;
    const running: any[] = [];
    if (isWorking) {
      if (depth > 0) {
        if (sheen) running.push(animate(breathEl, { opacity: 1 }, { duration: 0.2, ease: EASE_OUT as any }));
        if (glyphEl && glyph !== 'orb') {
          const lead = animate(glyphEl, { opacity: trough }, { duration: 0.2, ease: EASE_OUT as any });
          running.push(lead);
          lead.then(() => {
            if (cancelled) return;
        ...
        if (glyphEl) running.push(animate(glyphEl, { opacity: 1 }, { duration: 0.2, ease: EASE_OUT as any }));
        running.push(animate(breathEl, { opacity: 1 }, { duration: 0.2, ease: EASE_OUT as any }));
      }
    } else {
      if (glyphEl) running.push(animate(glyphEl, { opacity: GLYPH_DONE }, { duration: s, ease: EASE_OUT as any }));
      running.push(animate(breathEl, { opacity: 1 }, { duration: s, ease: EASE_OUT as any }));
    }
    return () => {
      cancelled = true;
    ...
    <>
      {glyph !== 'none' ? (
        <span ref={glyphRef} className="thought-line__glyph" aria-hidden="true">
          {glyph === 'orb' ? (
            <ThinkingOrb state={isWorking ? orbState : 'breathing'} size={20} paused={!isWorking} />
          ) : glyph === 'sparkle' ? (
