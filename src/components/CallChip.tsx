...
export default function CallChip({
  ...
  const roll = useRef({ cur: glyphOf(status), prev: null as string | null });
  ...
  }, [status, showTimer, name, argument]);
    ...
    >
      ...
      <span className="call-chip__slot" aria-hidden="true">
        <span className="call-chip__glyph" data-state={glyphState('tool')}>
          {toolIcon ? (
            React.isValidElement(toolIcon) ? toolIcon : (() => {
              const ToolIconComp = toolIcon;
              return <ToolIconComp size={iconSize} strokeWidth={1.8} />;
            })()
          ) : icon}
        </span>
