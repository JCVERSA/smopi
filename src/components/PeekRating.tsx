...
const SHAPES: Record<string, any> = { star: Star, heart: Heart, bolt: Zap };
...
export interface PeekRatingProps {
  ...
  onPreview?: (val: number | null) => void;
  count?: number;
...
export default function PeekRating({
  ...
  const st = useRef<{
    ...
    rect: DOMRect | null;
    rtl: boolean;
  }>({ hover: null, pressing: false, pointerId: null, settled: false, rect: null, rtl: false });
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    switch (e.key) {
      ...
      case ' ':
      case 'Enter': {
        const index = starEls.current.indexOf(e.target as any);
        if (index === -1) return;
    ...
    commit(next, false);
    (starEls.current[Math.max(next, 1) - 1] as HTMLElement)?.focus();
  };
          ...
          return (
            ...
            >
              ...
              >
                ...
                >
                  {icon ?? <IconComponent size={size} fill="currentColor" strokeWidth={1.5} />}
                </span>
              </span>
            </StarElement>
          );
